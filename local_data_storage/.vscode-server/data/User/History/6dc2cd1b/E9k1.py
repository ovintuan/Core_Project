...
def createSparkSession():
    ...
    jar_files = list_jar_files('/container/pyspark_workspace/local_data_storage/spark/jars/')
    spark_submit_local_jars_str = ','.join(jar_files)
    print('spark_submit_local_jars_str:\n {spark_submit_local_jars_str}')
    ...
    spark = SparkSession.builder.appName(session_id) \
        .master("spark://spark-master:7077")\
        .config("spark.jars", '/container/pyspark_workspace/local_data_storage/spark/jars/*')\
        .config("spark.sql.extensions", spark_submit_extensions_str) \
        .config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog")\
        .getOrCreate()
    sqlContext = SparkSession(spark)
