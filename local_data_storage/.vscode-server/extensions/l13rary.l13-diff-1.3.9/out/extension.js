...
class DiffCompare {
    ...
    initCompare(data) {
        ...
        if (statA.isFile() && statB.isFile())
            ...
            this.onError(`The left path (${getFileType(statA)}) and right path (${getFileType(statB)}) can't be compared! You can only compare a folder with a folder or a file with a file.`, pathA, pathB);
    }
    updateDiffs(data) {
